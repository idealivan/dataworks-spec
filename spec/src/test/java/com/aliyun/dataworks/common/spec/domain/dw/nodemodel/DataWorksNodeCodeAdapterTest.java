...
public class DataWorksNodeCodeAdapterTest {
    ...
    public void testXlab() {
        ...
        SpecScript scr = new SpecScript();
        SpecScriptRuntime rt = new SpecScriptRuntime();
        rt.setCommand(CodeProgramType.XLAB.name());
        scr.setRuntime(rt);
        scr.setContent(content);
        node.setScript(scr);
        DataWorksNodeCodeAdapter codeAdapter = new DataWorksNodeCodeAdapter(node);
    ...
    @Test
    public void testPaiflow() throws Exception {
        String spec = "{\n"
            + "\t\"version\":\"1.1.0\",\n"
            + "\t\"kind\":\"CycleWorkflow\",\n"
            + "\t\"spec\":{\n"
            ...
            + "\t\t\t\t\t\t\t\"type\":\"System\",\n"
            + "\t\t\t\t\t\t\t\"value\":\"${yyyymmdd}\",\n"
            + "\t\t\t\t\t\t\t\"id\":\"7420459281980958195\"\n"
            + "\t\t\t\t\t\t}\n"
            + "\t\t\t\t\t]\n"
            + "\t\t\t\t},\n"
            + "\t\t\t\t\"trigger\":{\n"
            + "\t\t\t\t\t\"type\":\"Scheduler\",\n"
            + "\t\t\t\t\t\"id\":\"8641186319777208557\",\n"
            + "\t\t\t\t\t\"cron\":\"0 0 0 * * ? *\",\n"
            + "\t\t\t\t\t\"startTime\":\"1970-01-01 00:00:00\",\n"
            + "\t\t\t\t\t\"endTime\":\"9999-01-01 00:00:00\",\n"
            + "\t\t\t\t\t\"timezone\":\"Asia/Shanghai\",\n"
            ...
            + "arguments:\\n      artifacts: []\\n      parameters: []\\n    container:\\n      image: pai-official-cn-hangzhou-registry-vpc"
            + ".cn-hangzhou.cr.aliyuncs.com/official/max-compute-executor:v20241210134900\\n      command:\\n      - bash\\n      - "
            + "/paiflow-bin/start.sh\\n      envs: {}\\n      volumeMounts:\\n      - name: pai-volume\\n        path: /pai\\n    dependencies: "
            + "[]\\n    initContainers:\\n    - image: pai-official-cn-hangzhou-registry-vpc.cn-hangzhou.cr.aliyuncs.com/official/paiflow-init:v1.0"
            + ".0\\n      command:\\n      - /bin/sh\\n      - -c\\n      - paiflow-init download --source "
            + "'oss://pai-studio-cn-hangzhou-prod/algorithm/pai/rag_sync_index/v1/afb415fb780f5cd50ca65d91b0e4d90e43972afd/working?endpoint=oss-cn"
            + "-hangzhou-internal.aliyuncs.com&roleARN=acs:ram::1326689413376250:role/pai-studio-algo-download-role&ownerId=1326689413376250'\\n   "
            ...
            + "\t\t\t\t\t\"owner\":\"WORKER_1482465063962\",\n"
            + "\t\t\t\t\t\"ownerName\":\"showName-WORKER_1482465063962\",\n"
            + "\t\t\t\t\t\"createTime\":\"2025-02-14 15:44:10\",\n"
            + "\t\t\t\t\t\"tenantId\":\"1\",\n"
            + "\t\t\t\t\t\"project\":{\n"
            + "\t\t\t\t\t\t\"mode\":\"STANDARD\",\n"
            + "\t\t\t\t\t\t\"projectId\":\"23620\",\n"
            ...
            + "\t},\n"
            + "\t\"metadata\":{\n"
            + "\t\t\"uuid\":\"8245048540225720582\"\n"
            + "\t}\n"
            + "}";

        Specification<DataWorksWorkflowSpec> specSpecification = SpecUtil.parseToDomain(spec);
        SpecNode paiflow = specSpecification.getSpec().getNodes().get(0);
        Assert.assertNotNull(paiflow);
        Assert.assertNotNull(paiflow.getPaiflow());

        SpecNode specNode = paiflow.getPaiflow().getNodes().get(0);
        ...
        codeAdapter.setContext(Context.builder().deployToScheduler(true).build());
        String codeToScheduler = codeAdapter.getCode();
        log.info("content deploy to scheduler: ");
        System.out.println(codeToScheduler);
    }
}
